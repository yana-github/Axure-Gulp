...
var IMPORT_RE = /^([ \t]*(?:\/\*.*)?)@import\s+["']([^"']+\*[^"']*(?:\.less)?)["'];?([ \t]*(?:\/[/*].*)?)$/gm;

function gulpLessGlob() {
        ...
        files.forEach(function (filename) {
          if (filename !== file.path && isLessFunc(filename)) {
            ...
            if (!ignorePaths.some(function (ignorePath) {
              ...
              // remove parent base path
              imports.push('@import "' + (0, _slash2.default)(filename) + '"' + ';');
            }
          }
        });
...
function isLessFunc(filename) {
  return !_fs2.default.statSync(filename).isDirectory() && _path2.default.extname(filename).match(/\.less/i);
}
